...
class AttentionUnet(BaseNetwork):
    ...
    def create_model(self, img_shape, kernel_size=5):
        ...
        attn1 = self.AttnGatingBlock(conv6, gating1, 224)
        up_samp1 = layers.Conv2DTranspose(224, kernel_size, strides=(2, 2), padding='same', activation="relu")(conv12)
        concat1 = layers.concatenate([up_samp1, attn1], axis=3)

        gating2 = self.UnetGatingSignal(concat1, is_batchnorm=True)
        attn2 = self.AttnGatingBlock(conv4, gating2, 112)
        up_samp2 = layers.Conv2DTranspose(112, kernel_size, strides=(2, 2), padding='same', activation="relu")(concat1)
