...
class CustomResUnet(BaseNetwork):
...
def combine_block(input_tensor, concat_tensor, filters):
    ...
    output = layers.concatenate([input_tensor, concat_tensor], axis=-1)
    output = layers.Conv2D(filters, kernel_size=1, activation="relu")(output)
    return output
...
def res_block(input_tensor, filters, kernel_size, dilations):
    ...
    for dilation in dilations:
        dilation_out = layers.Conv2D(filters, kernel_size=kernel_size, dilation_rate=dilation, activation="relu",
